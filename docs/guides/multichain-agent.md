...
// Agent gets identities on every supported chain
```
...
The agent infers from the asset:
...
When the asset exists on multiple chains, the agent asks:
...
Under the hood, a multichain agent is a single entity in the database with multiple chain registrations. The TEE derives different keys per chain using the derivation path:
...
The AI agent's system prompt includes all chain identities:

```
YOUR IDENTITY:
- Name: alice.wraith
- Chains: Horizen (0xabc...), Stellar (GABC...), Ethereum (0xdef...)
- Meta-Addresses:
  - Horizen: st:eth:0x...
  - Stellar: st:xlm:...
  - Ethereum: st:eth:0x...
...
When a chat message references a specific chain or asset, the agent routes the tool call to the appropriate chain connector. The chain connector registry handles all the routing:

```typescript
// Agent service internally does:
const connector = chainRegistry.get("stellar");
