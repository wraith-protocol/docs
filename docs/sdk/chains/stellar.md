...
```typescript
const signature = await stellarWallet.sign(
  Buffer.from(STEALTH_SIGNING_MESSAGE)
);
...
**Algorithm:** `SHA-256("wraith:tag:" || sharedSecret)[0]`
...
**Algorithm:** `SHA-256("wraith:scalar:" || sharedSecret)` interpreted as little-endian bigint, reduced mod L.
...
Note: the fourth argument is `spendingScalar` (bigint), not `spendingKey` like in the EVM module.
...
**Why this exists:** The stealth scalar `(spendingScalar + hashScalar) % L` is not necessarily clamped, so standard Stellar signing functions don't work.
...
- **Signing:** Must use `signWithScalar()` / `signStellarTransaction()` because stealth scalars aren't valid seeds.
