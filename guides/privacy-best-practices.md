...
Stealth addresses are one-time by design. The Wraith protocol enforces this — each payment generates a fresh address automatically. If you're building custom integrations with the [EVM crypto primitives](../sdk/chains/evm.md), call `generateStealthAddress()` for each payment.
