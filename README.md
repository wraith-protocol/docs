...
- [Getting Started](getting-started.md) — install, create an agent, send your first payment

### SDK Reference

- [SDK Overview](sdk/overview.md) — package structure, entry points, `Chain` enum
- [Agent Client](sdk/agent-client.md) — `Wraith` and `WraithAgent` class API
- [EVM Crypto Primitives](sdk/chains/evm.md) — low-level secp256k1 stealth address functions
- [Stellar Crypto Primitives](sdk/chains/stellar.md) — low-level ed25519 stealth address functions

### Architecture

- [Architecture Overview](architecture/overview.md) — system diagram, modules, database schema
- [Chain Connectors](architecture/chain-connectors.md) — `ChainConnector` interface, adding chains
- [TEE Security](architecture/tee.md) — key derivation, attestation, privacy model

### Guides

- [Single-Chain Agent](guides/single-chain-agent.md) — full lifecycle on one chain
- [Multichain Agent](guides/multichain-agent.md) — one name, multiple chains
- [Bring Your Own Model](guides/bring-your-own-model.md) — use OpenAI/Claude instead of Gemini
- [Stealth Payments](guides/stealth-payments.md) — how stealth addresses work (with visuals)
- [Privacy Best Practices](guides/privacy-best-practices.md) — scoring, what to avoid

### Contracts

- [EVM Contracts](contracts/evm.md) — Solidity contract specs and deployment
- [Stellar Contracts](contracts/stellar.md) — Soroban contract specs and deployment

### API Reference

- [HTTP Endpoints](api-reference/endpoints.md) — full TEE server API
- [TypeScript Types](api-reference/types.md) — all SDK types
